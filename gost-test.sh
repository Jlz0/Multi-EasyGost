...
function Install_ct() {
  ...
  if [[ ${addyn} == [Yy] ]]; then
    ...
    mkdir /etc/gost && wget --no-check-certificate https://cdn.paimom.cn/Multi-EasyGost/config.json && mv config.json /etc/gost && chmod -R 777 /etc/gost
  else
    ...
    wget --no-check-certificate https://raw.githubusercontent.com/Jlz0/Multi-EasyGost/master/gost.service && chmod -R 777 gost.service && mv gost.service /usr/lib/systemd/system
    mkdir /etc/gost && wget --no-check-certificate https://raw.githubusercontent.com/Jlz0/Multi-EasyGost/master/config.json && mv config.json /etc/gost && chmod -R 777 /etc/gost
  fi
...
function read_d_ip() {
  if [ "$flag_a" == "ss" ]; then
    ...
    echo -e "具体IP地址的填写, 取决于接收该流量的服务正在监听的IP(详见: https://github.com/Jlz0/Multi-EasyGost)"
    if [[ ${is_cert} == [Yy] ]]; then
...
update_sh() {
  ol_version=$(curl -L -s --connect-timeout 5 https://raw.githubusercontent.com/Jlz0/Multi-EasyGost/master/gost.sh | grep "shell_version=" | head -1 | awk -F '=|"' '{print $3}')
  if [ -n "$ol_version" ]; then
    if [[ "$shell_version" != "$ol_version" ]]; then
      ...
      [yY][eE][sS] | [yY])
        wget -N --no-check-certificate https://raw.githubusercontent.com/Jlz0/Multi-EasyGost/master/gost.sh
        echo -e "更新完成"
        exit 0
        ;;
      *) ;;
...
echo && echo -e "                 gost 一键安装配置脚本"${Red_font_prefix}[${shell_version}]${Font_color_suffix}"
  ...
  帮助文档：https://github.com/Jlz0/Multi-EasyGost
